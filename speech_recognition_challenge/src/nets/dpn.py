...
class InputBlock(nn.Module):
    def __init__(self, num_init_features, kernel_size=7,
                 padding=3, activation_fn=nn.ReLU(inplace=True), in_feats = 3):
        super(InputBlock, self).__init__()
        self.conv = nn.Conv2d(
            in_feats, num_init_features, kernel_size=kernel_size, stride=2, padding=padding, bias=False)
...
class DPN92(nn.Module):
    def __init__(self):
        ...
        model = DPN(num_init_features=64, k_r=96, groups=32,
                k_sec=(3, 4, 20, 3), inc_sec=(16, 32, 24, 128),
                num_classes=1000, test_time_pool=True)
        self.num_classes = 32#12

        self.model =  model
        model_list = [InputBlock(num_init_features = 64, kernel_size=7, padding=3, in_feats =1)] + list(self.model.features.children())[1:]
        self.model = nn.Sequential(*model_list)  
        self.avg = nn.AvgPool2d(4, count_include_pad=False) #avgpool_1a
        self.linear = nn.Linear(2688 , self.num_classes)
